    ...
    def save_tweets(self, tweets):
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                print "*Time: %s" % (t.strftime(fmt))
                print "*user_name: %s" % (tweet['user_name'])
                print "*text: %s" % (tweet['text'])
                print "*tweet_id: %s" % (tweet['tweet_id'])
                print "*user_id: %s" % (tweet['user_id'])
                print "*user_screen_name: %s" % (tweet['user_screen_name'])
                print "*created_at: %s" % (tweet['created_at'])
                print "*retweets: %s" % (tweet['retweets'])
                print "*favorites: %s" % (tweet['favorites'])

            # When reached max limit, return False so collection stops
...
if __name__ == '__main__':
    ...
    twit = TwitterSearchImpl(3, 5, 100)
    twit.search("I was a construction worker")